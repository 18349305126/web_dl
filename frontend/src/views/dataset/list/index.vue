<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <m-level-select v-model="listQuery.model" :tree-data="option.modelOptions" class="filter-item" style="width: 195px" />
...
<script>
import { fetchList, fetchPv, createDataset, updateDataset, deleteDataset } from '@/api/dataset'
...
export default {
  ...
  data() {
    return {
      ...
      option: {
        ...
        modelOptions: [{ label: 'CT', key: 'CT', children: [{ label: 'CT1', key: 'CT1' }, { label: 'CT1C', key: 'CT1C' }] }, { label: 'MRI', key: 'MRI', children: [{ label: 'T1', key: 'T1' }, { label: 'T1C', key: 'T1C' }, { label: 'T1DIXONC', key: 'T1DIXONC' }, { label: 'T2', key: 'T2' }] }, { label: 'CBCT', key: 'CBCT', children: [] }]
      },
      showReviewer: false,
      temp: {
        id: undefined,
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.$router.push({ path: `/dataset/edit/${row.id}` })
    },
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
    ...
    handleDelete(row, index) {
      deleteDataset(index).then(response => {
