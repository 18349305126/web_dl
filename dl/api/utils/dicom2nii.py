...
def DcmToNii(PathDicom, SaveRawDicom, segment=False):
    lstFilesDCM = []
    ...
    # 第七步：将现在的numpy数组通过SimpleITK转化为nii文件
    ...
    # p, d = os.path.split(p)
    if segment:
        sv_path = os.path.join(SaveRawDicom, d + "_segmentation" + ".nii")
    else:
        sv_path = os.path.join(SaveRawDicom, d + ".nii")
    SimpleITK.WriteImage(sitk_img, sv_path)


# path: the path of dcm
# sv_path: the path of segment
# sv_raw_path: the path of nii
def transAll(path, sv_path, sv_raw_path):
    cls = cfg.CATEGORY
    #path = 'Z:\MRI\data\T1seg'
    mkdir(os.path.join(sv_path))
    print(path + ' is being converted to nii!')
    im, gt = GetImageAndGT(path, cls, True, sv_path)
    if os.path.isdir(os.path.join(sv_path)):
        mkdir(os.path.join(sv_raw_path))
        DcmToNii(path, sv_raw_path)
        DcmToNii(sv_path, sv_raw_path, segment=True)
        print(path + ' has been converted to nii!')
...
class CheckPoint(object):
    def __init__(self, dir='./', nii_list=None):
        if nii_list==None:
            self.nii_dict={}
        else:
            self.nii_dict = nii_list
        self.dir = dir
        self.load_point()

    def save_point(self):
        json_str = json.dumps(self.nii_dict, indent=2)
        save_file = os.path.join(self.dir, "check_point.json")
        with open(save_file, "w") as f:
            f.write(str(json_str))

    def check_nii(self, nii):
        return check_dict(nii, self.nii_dict.copy())

    def load_point(self):
        save_file = os.path.join(self.dir, "check_point.json")
        try:
            with open(save_file, "r") as f:
                self.nii_dict = json.load(f)
        except IOError:
...
def dcm2nii():
    path_all = cfg.DICOM_DIR
    # print(path_all)
    save_all = cfg.NII_DIR
    seq_cls = cfg.SEQUENCE
    seg_path = cfg.SEGMENTAION_PATH
    ...
    for pathi in dir_list:
        ...
        transAll(path, sv_path, sv_raw_path)
